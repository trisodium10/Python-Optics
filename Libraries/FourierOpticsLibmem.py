...
"""
Created from FourierOpticsLib.py
Aug. 17, 2016
Designed to provide the same functionality as FourierOpticsLib but with lower
memory overhead in the coordinate grids.  This makes it slower, but it can
still run relatively large grids without exceeding memory limits.

Spatial and Frequency coordinates are stored as 1D arrays.  Meshes of these
coordinates have to be recalculated every time they are needed.
...
#            Nx1D = np.nanmin([2**(np.ceil(np.log2(xset[0]/xset[1]))),Nmax/2])
            Nx1D = np.nanmin([np.ceil(xset[0]/xset[1]),Nmax/2])
...
#                Ny1D = np.nanmin([2**(np.ceil(np.log2(yset[0]/yset[1]))),Nmax/2])
                Ny1D = np.nanmin([np.ceil(yset[0]/yset[1]),Nmax/2])
...
#            NewXset = (self.x[-1,-1],self.dx)
    ...
    def fr(self):
        fx1,fy1 = self.freq_mesh()
        gc.collect()
        frad = np.sqrt(fx1**2+fy1**2)
        del fx1
        del fy1
        gc.collect()
        return frad
    ...
    def space_mesh(self):
        ...
        return xM,yM
    def freq_mesh(self):
        fxM,fyM = np.meshgrid(self.fx,self.fy)
        return fxM,fyM
"""
...
#        self.fy = fy
    ...
    def propagate(self,distance):
        ...
        if np.abs(distance) > self.wavelength:
            self.field = OpticsFFT(self.field)
            ...
            self.field = self.field*H
            del H
            gc.collect()
            self.field = OpticsIFFT(self.field)
    ...
    def propagate_Fresnel(self,distance,trim=True):
        ...
        Nxh = np.min([np.round(self.wavelength*self.grid.fx[-1]*distance/self.grid.dx),self.grid.Nx/2])
        Nyh = np.min([np.round(self.wavelength*self.grid.fy[-1]*distance/self.grid.dy),self.grid.Ny/2]);
...
#            Outfield = scipy.signal.convolve2d(self.field,hxy,mode='full')*self.grid.dx*self.grid.dy 
    ...
    def translate_FT(self,xshift,yshift):
        fx,fy = self.grid.freq_mesh()
        gc.collect()
        self.field = self.spatial_filter(np.exp(1j*2*np.pi*(xshift*fx+yshift*fy)))
        del fx
        del fy
        gc.collect()
...
#                plt.title('Intensity')
        ...
        if len(savefile) > 0:
            ...
            plt.show();
        return fig1
    def imshow(self,coord='spatial',savefile='',title=''):
        # plot the intensity on an implot
        if coord=='spatial':
            # spatial intensity
            fig1 = plt.figure();
            Ifield = np.abs(self.field)**2
            plt.imshow(Ifield,extent=(self.grid.x.min(),self.grid.x.max(),self.grid.y.min(),self.grid.y.max()))
            plt.xlabel('x')
            plt.ylabel('y')
            plt.title(title)
        else:
            ...
            if coord=='angle':
                ...
                plt.imshow(Ifield,extent=(self.grid.fx.min()*self.wavelength,self.grid.fx.max()*self.wavelength,self.grid.fy.min()*self.wavelength,self.grid.fy.max()*self.wavelength))
                plt.xlabel(r'$\alpha_x$')
                plt.ylabel(r'$\alpha_y$')
            else:
                plt.imshow(Ifield,extent=(self.grid.fx.min(),self.grid.fx.max(),self.grid.fy.min(),self.grid.fy.max()))
                plt.xlabel('$f_x$')
                plt.ylabel('$f_y$')
            plt.title(title)
    ...
    def line(self,axis='x',position=0.0):
        # generate a line section of the electric field
        # default returns section along x at y = 0
        # accepts axis = 0 (y), 1 (x), 'x','y','fx','fy'
        if axis == 1 or axis == 'x':
            iposition = np.argmin(np.abs(self.grid.y-position))
            return self.field[iposition,:]
        elif axis == 0 or axis == 'y':
            iposition = np.argmin(np.abs(self.grid.x-position))
            return self.field[:,iposition]
        elif axis == 'fx':
            iposition = np.argmin(np.abs(self.grid.fy-position))
            return OpticsFFT(self.field)[iposition,:]
        elif axis == 'fy':
            iposition = np.argmin(np.abs(self.grid.fx-position))
            return OpticsFFT(self.field)[:,iposition]
        else:
            print('Efield.line:  axis argument not recognized')
    def plot(self,axis='x',position=0,fignum=np.nan):
        if axis == 1 or axis == 'x':
            pltaxis = self.grid.x
            axistxt = 'x'
        elif axis == 0 or axis == 'y':
            ...
            pltaxis = self.grid.fx
            axistxt = '$f_x$'
        elif axis == 'fy':
            pltaxis = self.grid.fy
            axistxt = '$f_y$'
        else:
...
class Surface:
    ...
    def mask(self,grid,wavelength):
        x,y = grid.freq_mesh()
        return np.exp(1j*2*(self.n2-self.n1)*np.pi/wavelength*(self.tiltx*x+self.tilty*y))
...
class SphericalParticle:
    ...
    def freq_mask(self,grid,wavelength,angle_offset=[0.0,0.0]):
        ...
        if angle_offset==[0,0]:
            ...
            scat_ang = np.sqrt((fx*wavelength-angle_offset[0])**2 + \
                (fy*wavelength-angle_offset[1])**2)
...
#        print('Sp is %s long with %s rows and 0 cols'%(np.size(Sp[0,:]),np.shape(Sp[0,:]))) #[0],np.shape(Sp[0,:])[1]))
        ...
        Snormalize = -1/(np.cos(fx*wavelength)*np.cos(fy*wavelength))*np.sqrt(grid.dfx*grid.dfy)*wavelength**2/(2*np.pi)    
    ...
    def space_mask(self,grid,wavelength,offset=[0.0,0.0],angle_offset=[0.0,0.0]):
        ...
        % information in a global coordinate from such that
        % T = [Sh  Shv]
        %     [Shv Sv ]
        %
        % Interpreting Output:
        ...
        if offset[0] != 0 or offset[1] != 0:
            fx,fy = grid.freq_mesh()   
            LinearPhaseOffset = np.exp(1j*np.pi*(offset[0]*fx+offset[1]*fy))
...
#        self.finess_coeff = 1.0/np.sin(np.pi/(2*self.finess))
    ...
    def fourier_mask(self,grid,wavelength,transmit=True):
        ...
        """
        fx,fy = grid.freq_mesh()
        etalon_phase = 2*np.pi/wavelength*2*self.nE*self.length*np.cos(np.sqrt((fx*wavelength+self.tilt[0])**2+(fy*wavelength+self.tilt[1])**2))
        if transmit:
    ...
    def spectrum(self,lam_range0,InWavelength=True,aoi=0.0,transmit=True):
        ...
        etalon_phase = 2*np.pi/lam_range*2*self.nE*self.length*np.cos(aoi)
        if transmit:
...
def GaussianBeam(grid,radius,wavelength,divergence=np.nan,angle=0.0,n=1.0,z=0,direction=1,Norder=1.0):
    ...
    x,y = grid.space_mesh()    
    if angle != 0.0:
        xb = x*np.cos(angle)+y*np.sin(angle)
        yb = x*np.sin(angle)-y*np.cos(angle)   
    else:
        xb = x
        yb = y   
...
def TelescopeSpider(grid,Router,Rinner,offset = [0.0,0.0],width=0.0,rot=0.0):
    ...
    SpMask = Window(grid.r(),2*Router)* \
        Window(np.sqrt((x-offset[0])**2+(y-offset[1])**2),2*Rinner,invert=True)
    SpMask[np.nonzero(np.abs(x-offset[0])<=width/2.0)] = 0;
    SpMask[np.nonzero(np.abs(y-offset[1])<=width/2.0)] = 0;
    return SpMask
...
Simulate incoherent backscatter from an incident electric field by using the adding
...
to the system detecting the scattering (e.g. set angle limit to 1.5*FS or ASacceptance)
...
def animate_z(i,Ein,z_array):
    ...
    cont = plt.imshow(np.abs(Ein.field)**2,extent=(Ein.grid.x.min(),Ein.grid.x.max(),Ein.grid.y.min(),Ein.grid.y.max()))
