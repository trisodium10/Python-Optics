...
E2 = FO.Efield(lam,grid0)
E2.mask(np.exp(1j*np.pi*grid0.x/(20*grid0.dx)))

E3 = FO.Efield(lam,grid0)
E3.field = E1.field + E2.field

#E3.imshow()
#E3.plot(axis='x')

PhaseGrating = np.exp(1j*2*np.pi*np.mod(grid0.x/(10*grid0.dx),5e-6/lam))
plt.imshow(np.angle(PhaseGrating))

E3.mask(PhaseGrating)

E3.imshow(coord='freq')
E3.plot(axis='fx')
...
pow0 = E1.power()
part1.propagate(E1,offset=[0.0005,0.00025])
#E1.propagate(5e-2)
E1.imshow()
pow1 = E1.power()
r_est = np.sqrt((1-pow1/pow0)/np.pi)
